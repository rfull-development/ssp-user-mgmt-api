...
using UserManagementApi.Database.Extensions;
...
namespace UserManagementApi.Database.Clients.Postgres
{
    public class UserDatabaseClient(string connectionString, string schema) : DatabaseClient(schema), IUserDatabaseClient, IDisposable
    {
        ...
        static UserDatabaseClient()
        {
            ...
            SqlMapper.SetTypeMap(typeof(Email), new CustomPropertyTypeMap(typeof(Email), CustomMap));
        ...
        public async Task<int> CreateEmailAsync(Email email)
        {
            ...
            try
            {
                string table = GetTableName<Email>();
                ...
                VALUES
                    ({values});
                """;
                rows = await _connection.ExecuteAsync(query, email);
                await transaction.CommitAsync();
            }
            catch (Exception e)
            {
                await transaction.RollbackAsync();
                throw new DatabaseException("Failed to create email.", e);
            }
            return rows;
        }

        public async Task<List<Email>> GetEmailListAsync(long itemId)
        {
            if (itemId < 1)
            {
                throw new DatabaseParameterException(nameof(itemId));
            ...
            {
                string table = GetTableName<Email>();
                string columns = GenerateColumnListQuery<Email>();
                string query = $"""
                SELECT
                    {columns}
                FROM
                ...
                WHERE
                    "item_id" = @ItemId;
                """;
                IEnumerable<Email> results = await _connection.QueryAsync<Email>(query, new
                {
                    ItemId = itemId
                });
                emails = results.ToList();
            }
            catch (Exception e)
            {
                throw new DatabaseException("Failed to get email list.", e);
            }
            return emails;
        }
